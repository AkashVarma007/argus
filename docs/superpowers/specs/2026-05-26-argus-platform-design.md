...
interface Build {
  ...
  packageMeta: { name: string; version: string; description: string; license: string }
  nodes: BuildNode[]      // server + tools + prompts + resources, with positions
  edges: BuildEdge[]      // membership + dependency + prompt-uses-tool
  viewport: { x: number; y: number; zoom: number }  // restore canvas state
...
interface BuildNode {
  ...
  data: ToolDef | PromptDef | ResourceDef | ServerMeta | CapabilityInfo
}
...
### Composer model — node graph

A build is a structured document represented as a **spatial node graph**. The canvas is a pannable, zoomable workspace (`react-flow` or hand-rolled SVG/canvas) where each artifact is a node and conceptual relationships are edges.

**Node types**:
- **Server node** (single, anchored top-left) — package metadata (name, version, description, license, language target)
- **Tool node** — name, description, input schema, optional output schema
- **Prompt node** — name, description, argument list
- **Resource node** — URI scheme, MIME type, description, resolver behaviour
- **Capability node** — derived/auto, shows what server advertises (read-only, generated from connected nodes)

**Edges**:
- Each Tool/Prompt/Resource node connects back to the Server node by default (membership edge, drawn faint)
- Optional **dependency edges** between tools (`tool A calls tool B` — emits commentary in generated code, not runtime)
- Optional **prompt-uses-tool** edges (visualizes prompt → tool invocation chain)

**Interaction**:
- Drag from palette (left rail) into canvas to add a node
- Click node to open inspector (right rail) with structured form for schemas, descriptions, etc.
- Schemas edited in dual-mode: visual form OR text-mode JSON (toggleable)
- No raw source code editing
- Pan: space-drag or middle-mouse, zoom: cmd-scroll, fit-to-view: `f` key

**Persistence**: Node positions saved to build doc. Re-opening a build restores layout exactly.
...
| OQ6 | Build canvas: node-graph (Workbench) or stacked-list (simpler)?                            | **Resolved: node-graph v1**       |
