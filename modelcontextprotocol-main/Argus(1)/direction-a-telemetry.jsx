...
const ATok = {
  ...
  accent: '#7ae8ff',          // ice cyan — live state, the protagonist
  accentDim: '#173846',
  fail: '#ffb4a8',            // soft warning, low-volume — failures pop via brightness contrast
  paper: '#0c1015',           // page surface
  bg: '#06080b',              // chrome strip / deepest
  ink0: '#eef2f7',            // brightest text
  ink1: '#cdd3dc',
  ink2: '#9099a4',
  ink3: '#5d6470',
  ink4: '#3a414b',
  ink5: '#222831',            // hairline
  ink6: '#171c24',
  ink7: '#12161d',
  surface1: '#13181f',        // elevated card
  surface2: '#1b2129',        // input / raised
  glow: 'rgba(122,232,255,0.45)',
};
...
function AStrip({ scan = 'SCN-0142', state = 'IDLE', tool = '— · —', endpoint, hint }) {
  return (
    <div style={{
      height: 28, background: ATok.bg, color: ATok.ink2,
      fontFamily: ATok.mono, fontSize: 10.5, letterSpacing: '0.06em',
      display: 'flex', alignItems: 'center', padding: '0 14px', gap: 18,
      flexShrink: 0,
    }}>
      <span style={{ fontWeight: 700, letterSpacing: '0.18em', color: ATok.ink0 }}>ARGUS</span>
      ...
      <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}>
        <span style={{ color: ATok.ink4 }}>state</span>
        <LiveDot color={state === 'RUNNING' ? ATok.accent : ATok.ink2} size={6} dur={state === 'RUNNING' ? '0.9s' : '2.4s'} />
        <span style={{ color: state === 'RUNNING' ? ATok.accent : ATok.ink0 }}>{state}</span>
      </span>
      <span style={{ flex: 1 }} />
...
function ARail({ active = 'TEST' }) {
  ...
  return (
    <div style={{
      width: 64, background: ATok.bg, color: ATok.ink1,
      ...
      {items.map(([label, num]) => (
        <div key={label} style={{
          ...
          color: active === label ? ATok.surface1 : ATok.ink4,
        }}>
    ...
    <div style={{ width: '100%', height: '100%', background: ATok.paper, fontFamily: ATok.font, color: ATok.ink0, padding: 40, boxSizing: 'border-box', display: 'flex', flexDirection: 'column', gap: 28 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginTop: 4 }}>
        {[
          ...
          <div key={label} style={{ background: bg, height: 56, padding: '8px 10px', color: bg === ATok.ink0 ? ATok.surface1 : ATok.ink1, fontFamily: ATok.mono, fontSize: 10, letterSpacing: '0.12em', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
        ...
        <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '1.3fr 1fr', gap: 1, background: ATok.ink5 }}>
          ...
          <div style={{ background: ATok.paper, display: 'flex', flexDirection: 'column' }}>
            <AH kicker="01 · TEST" title="SCAN HISTORY" right={
              <button style={{ fontFamily: ATok.mono, fontSize: 10, letterSpacing: '0.16em', padding: '6px 12px', border: `1px solid ${ATok.ink0}`, background: ATok.accent, color: ATok.surface1 }}>NEW SCAN ↵</button>
            } />
            <div style={{ fontFamily: ATok.mono, fontSize: 10.5, lineHeight: 1, color: ATok.ink2 }}>
              ...
              {SCAN_HISTORY.map((s, i) => (
                <div key={s[0]} style={{ display: 'grid', gridTemplateColumns: '70px 1fr 90px 50px 32px 50px', padding: '10px 18px', borderBottom: `1px solid ${ATok.ink6}`, background: i === 0 ? ATok.surface1 : 'transparent' }}>
      ...
      <div style={{ flex: 1, display: 'flex' }}>
        ...
        <div style={{ flex: 1, padding: '24px 32px', overflow: 'hidden' }}>
          ...
          <div style={{ marginTop: 22, display: 'flex', gap: 12, alignItems: 'center' }}>
            <button style={{ fontFamily: ATok.mono, fontSize: 12, letterSpacing: '0.18em', padding: '12px 22px', border: 'none', background: ATok.accent, color: ATok.surface1 }}>RUN SCAN ↵</button>
        ...
        <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '1fr 320px', gap: 1, background: ATok.ink5, minHeight: 0 }}>
          ...
          <div style={{ background: ATok.paper, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
            ...
            <div style={{ padding: '14px 18px', display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 18, borderBottom: `1px solid ${ATok.ink5}` }}>
              {[
                ['RUN', '107/243', ATok.surface1, ATok.accent],
                ['PASS', '94', ATok.surface1, ATok.ink0],
                ['FAIL', '13', ATok.surface1, ATok.ink0],
                ['SKIP', '0', ATok.surface1, ATok.ink3],
              ].map(([k,v,bg,c],i) => (
            ...
            <div style={{ padding: '12px 18px', borderBottom: `1px solid ${ATok.ink5}`, position: 'relative', overflow: 'hidden' }}>
              {/* sweeping highlight bar — runs across the meter */}
              <div style={{
                position: 'absolute', top: 26, bottom: 14, width: '22%',
                background: `linear-gradient(90deg, transparent, ${ATok.accent}26, transparent)`,
                animation: 'argus-sweep 3.4s linear infinite', pointerEvents: 'none',
              }} />
          ...
          <div style={{ background: ATok.paper, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            ...
            <div style={{ padding: '14px 18px', fontFamily: ATok.mono, fontSize: 10.5, color: ATok.ink2 }}>
              ...
              <pre style={{ margin: '4px 0 0', padding: '8px 10px', background: ATok.surface1, border: `1px solid ${ATok.ink6}`, fontSize: 10, overflow: 'hidden' }}>{`POST /mcp
auth: Bearer …2c8a
{ "method": "tools/call",
  "params": { "name":
    "repo.read_file" } }`}</pre>
    ...
    <div style={{ width: '100%', height: '100%', background: ATok.paper, display: 'flex', flexDirection: 'column', fontFamily: ATok.font }}>
      ...
      <div style={{ flex: 1, display: 'flex', minHeight: 0 }}>
        ...
        <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '300px 1fr', gap: 1, background: ATok.ink5 }}>
          {/* Grade reveal panel */}
          <div style={{
            background: ATok.bg, color: ATok.ink1, padding: '24px 22px',
            display: 'flex', flexDirection: 'column', position: 'relative', overflow: 'hidden',
            borderRight: `1px solid ${ATok.ink5}`,
          }}>
            {/* subtle scanline texture */}
            <div style={{
              position: 'absolute', inset: 0, pointerEvents: 'none',
              background: `repeating-linear-gradient(0deg, transparent 0 3px, rgba(122,232,255,0.025) 3px 4px)`,
            }} />
            ...
            <div style={{ marginTop: 24, display: 'flex', alignItems: 'baseline', gap: 8, position: 'relative' }}>
              {/* radial glow behind the grade */}
              <div style={{
                position: 'absolute', left: -40, top: -40, width: 280, height: 280,
                background: `radial-gradient(circle, ${ATok.accent}33 0%, transparent 55%)`,
                pointerEvents: 'none', animation: 'argus-breathe 4s ease-in-out infinite',
              }} />
              <div style={{
                fontFamily: ATok.mono, fontWeight: 700, fontSize: 156, lineHeight: 0.82,
                letterSpacing: '-0.05em', color: ATok.accent,
                position: 'relative', zIndex: 1,
                ['--g']: ATok.glow,
                animation: 'argus-glow 4s ease-in-out infinite',
              }}>B+</div>
              <div style={{ position: 'relative', zIndex: 1 }}>
                <div style={{ fontFamily: ATok.mono, fontSize: 26, fontWeight: 700, color: ATok.ink0 }}>229</div>
                <div style={{ fontFamily: ATok.mono, fontSize: 11, color: ATok.ink3 }}>of 243</div>
              </div>
            </div>

            <div style={{ marginTop: 6, fontFamily: ATok.mono, fontSize: 11, color: ATok.ink2 }}>
              ↑ <span style={{ color: ATok.accent }}>+8</span> from SCN-0141 · B
            </div>

            <div style={{ marginTop: 28, fontFamily: ATok.mono, fontSize: 10.5, color: ATok.ink2, lineHeight: 1.7, borderTop: `1px solid ${ATok.ink2}`, paddingTop: 16 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginTop: 18 }}>
              {['DOWNLOAD HTML', 'DOWNLOAD JSON', 'DOWNLOAD MD'].map((t) => (
                <button key={t} style={{ fontFamily: ATok.mono, fontSize: 10, letterSpacing: '0.16em', padding: '9px 12px', border: `1px solid ${ATok.ink3}`, background: 'transparent', color: ATok.ink2, textAlign: 'left' }}>↓ {t}</button>
              ))}
              <button style={{ marginTop: 6, fontFamily: ATok.mono, fontSize: 10, letterSpacing: '0.16em', padding: '9px 12px', border: 'none', background: ATok.accent, color: ATok.paper, fontWeight: 700 }}>RE-SCAN ↵</button>
            </div>
          </div>
          ...
          <div style={{ background: ATok.paper, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            ...
            <div style={{ flex: 1, overflow: 'hidden', fontFamily: ATok.mono, fontSize: 11 }}>
              {CATEGORIES.map(([name, total, fail], i) => {
                ...
                return (
                  <div key={name} style={{ display: 'grid', gridTemplateColumns: '24px 1.4fr 1.6fr 60px 60px', padding: '8px 18px', borderBottom: `1px solid ${ATok.ink6}`, alignItems: 'center', background: fail > 0 ? ATok.surface1 : 'transparent' }}>
      ...
      <div style={{ flex: 1, display: 'flex', minHeight: 0 }}>
        ...
        <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '360px 1fr', gap: 1, background: ATok.ink5, minHeight: 0 }}>
            ...
            <div style={{ padding: '8px 0', fontFamily: ATok.mono, fontSize: 10.5 }}>
              {CHECKS.Auth.map((c, i) => (
                <div key={c[0]} style={{
                  ...
                  background: i === 0 ? ATok.surface1 : 'transparent', borderBottom: `1px solid ${ATok.ink6}`,
                }}>
          ...
          <div style={{ background: ATok.paper, padding: '20px 26px', overflow: 'hidden', fontFamily: ATok.mono, fontSize: 11.5, color: ATok.ink2, display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ marginTop: 6, padding: '12px 14px', borderLeft: `3px solid ${ATok.ink0}`, background: ATok.surface1, fontFamily: '"Newsreader", serif', fontSize: 13, lineHeight: 1.55, color: ATok.ink1, fontStyle: 'italic' }}>
            ...
            <div style={{ marginTop: 16, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
              <div>
                ...
                <pre style={{ margin: '6px 0 0', padding: '10px 12px', background: ATok.surface1, border: `1px solid ${ATok.ink6}`, fontSize: 10.5, lineHeight: 1.5, overflow: 'hidden' }}>{`POST /mcp
...
# tool requires = ["write"]`}</pre>
              ...
              <div>
                ...
                <pre style={{ margin: '6px 0 0', padding: '10px 12px', background: ATok.surface1, border: `1px solid ${ATok.ink0}`, fontSize: 10.5, lineHeight: 1.5, overflow: 'hidden' }}>{`HTTP/1.1 200 OK
{ "result": {
    "content": [{
      "type": "text",
      "text": "wrote 3 lines"
...
# expected -32002 insufficient_scope`}</pre>
            ...
            <pre style={{ margin: '6px 0 0', padding: '12px 14px', background: ATok.bg, color: ATok.ink1, fontSize: 10.5, lineHeight: 1.6, overflow: 'hidden' }}>{`// before tools/call dispatch
  ...
  return rpc.error(-32002, 'insufficient_scope', {
    required, granted
            ...
            <div style={{ marginTop: 'auto', paddingTop: 14, display: 'flex', gap: 12, alignItems: 'center', fontSize: 10, color: ATok.ink3 }}>
              <button style={{ fontFamily: ATok.mono, fontSize: 10, letterSpacing: '0.14em', padding: '8px 14px', border: `1px solid ${ATok.ink0}`, background: ATok.accent, color: ATok.surface1 }}>OPEN IN LEARN · §7.4 →</button>
      ...
      <div style={{ flex: 1, display: 'flex', minHeight: 0 }}>
        ...
        <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '220px 1fr 340px', gap: 1, background: ATok.ink5, minHeight: 0 }}>
          ...
          <div style={{ background: ATok.paper, overflow: 'hidden' }}>
            ...
            <div style={{ padding: '12px 16px', fontFamily: ATok.mono, fontSize: 10.5 }}>
              ...
              {TOOLS_FOR_BUILD.map((t,i) => (
                <div key={t[0]} style={{ padding: '7px 8px', marginTop: 2, background: i===1 ? ATok.surface1 : 'transparent', borderLeft: i===1 ? `2px solid ${ATok.ink0}` : '2px solid transparent', color: i===1 ? ATok.ink0 : ATok.ink2, fontWeight: i===1?600:400 }}>
                  {t[0]}
                </div>
              ))}
          ...
          <div style={{ background: ATok.paper, padding: '20px 26px', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ marginTop: 22, display: 'flex', alignItems: 'stretch', gap: 0, fontFamily: ATok.mono, fontSize: 10.5 }}>
              {[
                ...
                <React.Fragment key={label}>
                  <div style={{ flex: 1, padding: 12, border: `1px solid ${ATok.ink3}`, background: i === 3 ? ATok.surface1 : ATok.paper, position: 'relative' }}>
            ...
            <pre style={{ marginTop: 6, padding: '12px 14px', background: ATok.surface1, border: `1px solid ${ATok.ink5}`, fontFamily: ATok.mono, fontSize: 11, lineHeight: 1.55, color: ATok.ink1, flex: 1, overflow: 'hidden' }}>{`export async function repoReadFile(args, ctx) {
    ...
    bytes:   raw.byteLength,
          ...
          <div style={{ background: ATok.paper, overflow: 'hidden' }}>
            ...
            <div style={{ padding: '14px 18px', fontFamily: ATok.mono, fontSize: 10.5, color: ATok.ink2 }}>
              ...
              <input defaultValue="repo.read_file" style={{ width: '100%', marginTop: 4, padding: '6px 8px', border: `1px solid ${ATok.ink4}`, fontFamily: ATok.mono, fontSize: 11, background: ATok.surface1, boxSizing: 'border-box' }} />

              <div style={{ color: ATok.ink3, fontSize: 9, letterSpacing: '0.16em', marginTop: 14 }}>DESCRIPTION</div>
              <textarea defaultValue="Read a single file from a GitHub repository at the given ref." style={{ width: '100%', marginTop: 4, padding: '6px 8px', border: `1px solid ${ATok.ink4}`, fontFamily: ATok.mono, fontSize: 11, lineHeight: 1.4, background: ATok.surface1, minHeight: 50, boxSizing: 'border-box', resize: 'none' }} />

              <div style={{ color: ATok.ink3, fontSize: 9, letterSpacing: '0.16em', marginTop: 14 }}>PARAMETERS</div>
              <div style={{ marginTop: 6, border: `1px solid ${ATok.ink5}`, background: ATok.surface1 }}>
                {[
        ...
        <div style={{ flex: 1, padding: '24px 32px', display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: 32 }}>
          <div>
            ...
            <div style={{ marginTop: 20, border: `1px solid ${ATok.ink5}`, fontFamily: ATok.mono, fontSize: 10.5 }}>
              ...
              {files.map((f, i) => (
                <div key={f[0]} style={{ display: 'grid', gridTemplateColumns: '1fr 80px', padding: '6px 14px', borderBottom: i < files.length - 1 ? `1px solid ${ATok.ink6}` : 'none', alignItems: 'center', background: i % 2 ? 'transparent' : ATok.surface1 }}>
          ...
          <div>
            ...
            <div style={{ marginTop: 20, display: 'flex', flexDirection: 'column', gap: 8 }}>
              <button style={{ fontFamily: ATok.mono, fontSize: 12, letterSpacing: '0.18em', padding: '14px 18px', border: 'none', background: ATok.accent, color: ATok.surface1, textAlign: 'left' }}>↓ DOWNLOAD .ZIP &nbsp;·&nbsp; 11.7 kB</button>
            ...
            <div style={{ marginTop: 18, padding: '12px 14px', background: ATok.surface1, border: `1px solid ${ATok.ink5}`, fontFamily: ATok.mono, fontSize: 10.5, color: ATok.ink2, lineHeight: 1.6 }}>
        ...
        <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '240px 1fr 280px', gap: 1, background: ATok.ink5, minHeight: 0 }}>
          ...
          <div style={{ background: ATok.paper, overflow: 'hidden' }}>
            <div style={{ padding: '12px 14px', borderBottom: `1px solid ${ATok.ink5}` }}>
              <input placeholder="⌘K  search spec…" style={{ width: '100%', padding: '7px 10px', fontFamily: ATok.mono, fontSize: 11, border: `1px solid ${ATok.ink4}`, background: ATok.surface1, boxSizing: 'border-box' }} />
            </div>
            <div style={{ padding: '8px 0', fontFamily: ATok.mono, fontSize: 11 }}>
              {SPEC_TREE.map((s) => {
                ...
                return (
                  <div key={s[0]} style={{ padding: `3px 14px 3px ${14 + (depth-1)*14}px`, color: active ? ATok.ink0 : ATok.ink2, background: active ? ATok.surface1 : 'transparent', fontWeight: active ? 600 : 400, borderLeft: active ? `2px solid ${ATok.ink0}` : '2px solid transparent', display: 'flex', justifyContent: 'space-between' }}>
                    <span>{s[1]}</span>
          ...
          <div style={{ background: ATok.paper, padding: '24px 32px', overflow: 'hidden' }}>
            ...
            <pre style={{ marginTop: 6, padding: '14px 16px', background: ATok.bg, color: ATok.ink1, fontFamily: ATok.mono, fontSize: 11, lineHeight: 1.6 }}>{`{
  "error": {
    "code": -32002,
    ...
    "data": { "required": ["write"], "granted": ["read"] }
          ...
          <div style={{ background: ATok.paper, padding: '20px 18px', overflow: 'hidden' }}>
            ...
            <div style={{ marginTop: 22, padding: '12px 12px', background: ATok.surface1, border: `1px solid ${ATok.ink5}`, fontFamily: ATok.mono, fontSize: 10.5, color: ATok.ink2 }}>
              ...
              <button style={{ marginTop: 8, width: '100%', fontFamily: ATok.mono, fontSize: 10, letterSpacing: '0.14em', padding: '7px 10px', border: `1px solid ${ATok.ink0}`, background: ATok.accent, color: ATok.surface1 }}>← BACK TO REPORT</button>
            </div>
          </div>
        </div>
      </div>
        ...
        <div style={{ background: ATok.paper, padding: '40px 36px', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ marginTop: 'auto', marginBottom: 'auto' }}>
            ...
            <div style={{ marginTop: 20, display: 'flex', gap: 8 }}>
              <input placeholder="ws:// http:// stdio://" style={{ flex: 1, padding: '12px 14px', fontFamily: ATok.mono, fontSize: 13, border: `1px solid ${ATok.ink2}`, background: ATok.surface1, boxSizing: 'border-box' }} />
              <button style={{ fontFamily: ATok.mono, fontSize: 11, letterSpacing: '0.16em', padding: '12px 18px', border: 'none', background: ATok.accent, color: ATok.surface1 }}>SCAN ↵</button>
            </div>
        ...
        <div style={{ background: ATok.paper, padding: '40px 36px', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ marginTop: 24 }}>
            ...
            <div style={{ marginTop: 22, display: 'flex', gap: 8 }}>
              <button style={{ fontFamily: ATok.mono, fontSize: 11, letterSpacing: '0.16em', padding: '10px 16px', border: 'none', background: ATok.accent, color: ATok.surface1 }}>RETRY ↵</button>
