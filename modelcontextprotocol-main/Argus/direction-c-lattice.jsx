...
function Cell({ state, dim, size = 12, gap = 2 }) {
  const base = {
    width: size, height: size, flexShrink: 0,
  };
  if (state === 'pass') return <div style={{ ...base, background: CTok.green, opacity: dim ? 0.18 : 1 }} />;
  if (state === 'fail') return <div style={{ ...base, background: CTok.ink0, opacity: dim ? 0.25 : 1, boxShadow: dim ? 'none' : `inset 0 0 0 1.5px ${CTok.ink0}` }} />;
  if (state === 'skip') return <div style={{ ...base, background: 'transparent', border: `1px solid ${CTok.ink4}`, boxSizing: 'border-box' }} />;
  if (state === 'run')  return <div style={{ ...base, background: `repeating-linear-gradient(45deg, ${CTok.ink0} 0 2px, transparent 2px 4px)` }} />;
  if (state === 'pending') return <div style={{ ...base, background: CTok.ink6 }} />;
  return <div style={{ ...base, background: CTok.ink6 }} />;
}
    ...
    <div style={{ width: '100%', height: '100%', background: CTok.paper, display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, padding: '28px 40px', display: 'grid', gridTemplateColumns: '1.4fr 1fr', gap: 36, minHeight: 0, overflow: 'hidden' }}>
        <div>
          ...
          <div style={{ marginTop: 22, padding: '20px 22px', background: '#fff', border: `1px solid ${CTok.ink5}` }}>
            <Lattice cellSize={10} dim={false} />
          </div>

          <div style={{ marginTop: 22, display: 'flex', gap: 10 }}>
            <button style={{ fontFamily: CTok.sans, fontSize: 14, fontWeight: 600, padding: '10px 20px', background: CTok.ink0, color: '#fff', border: 'none' }}>Run again ↵</button>
            <button style={{ fontFamily: CTok.sans, fontSize: 14, fontWeight: 500, padding: '10px 20px', background: '#fff', color: CTok.ink1, border: `1px solid ${CTok.ink4}` }}>New endpoint</button>
            <button style={{ fontFamily: CTok.sans, fontSize: 14, fontWeight: 500, padding: '10px 20px', background: '#fff', color: CTok.ink1, border: `1px solid ${CTok.ink4}` }}>Open SCN-0142</button>
          </div>
        </div>
    ...
    <div style={{ width: '100%', height: '100%', background: CTok.paper, display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, padding: '32px 40px', display: 'grid', gridTemplateColumns: '1fr 320px', gap: 36, minHeight: 0 }}>
        ...
        <div>
          ...
          <div style={{ marginTop: 20, display: 'grid', gridTemplateColumns: '1fr 220px', gap: 0, border: `1px solid ${CTok.ink2}`, background: '#fff' }}>
            <input defaultValue="localhost:3845/mcp" style={{ padding: '18px 20px', fontFamily: CTok.mono, fontSize: 17, border: 'none', outline: 'none', background: '#fff' }} />
            <button style={{ background: CTok.ink0, color: '#fff', fontFamily: CTok.sans, fontSize: 15, fontWeight: 600, border: 'none' }}>Run scan ↵</button>
          </div>
        ...
        <div style={{ background: '#fff', border: `1px solid ${CTok.ink5}`, padding: '18px 18px' }}>
    ...
    <div style={{ width: '100%', height: '100%', background: CTok.paper, display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '1fr 340px', minHeight: 0 }}>
        ...
        <div style={{ padding: '20px 40px 22px', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ marginTop: 22, flex: 1, padding: '20px 22px', background: '#fff', border: `1px solid ${CTok.ink5}`, overflow: 'auto' }}>
            <Lattice scanProgress={progress} cellSize={14} />
          </div>
        </div>

        {/* Right rail: log */}
        <div style={{ background: '#fff', borderLeft: `1px solid ${CTok.ink5}`, padding: '18px 20px', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
            <button style={{ flex: 1, fontFamily: CTok.sans, fontSize: 12.5, fontWeight: 600, padding: '8px 0', background: '#fff', color: CTok.ink1, border: `1px solid ${CTok.ink3}` }}>Pause</button>
            <button style={{ flex: 1, fontFamily: CTok.sans, fontSize: 12.5, fontWeight: 600, padding: '8px 0', background: '#fff', color: CTok.ink0, border: `1px solid ${CTok.ink0}` }}>Abort</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <div style={{ width: '100%', height: '100%', background: CTok.paper, display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ padding: '14px 40px 10px', display: 'flex', alignItems: 'baseline', gap: 24, borderBottom: `1px solid ${CTok.ink5}` }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button style={{ fontFamily: CTok.sans, fontSize: 12.5, padding: '6px 12px', background: '#fff', color: CTok.ink1, border: `1px solid ${CTok.ink3}` }}>↓ html</button>
          <button style={{ fontFamily: CTok.sans, fontSize: 12.5, padding: '6px 12px', background: '#fff', color: CTok.ink1, border: `1px solid ${CTok.ink3}` }}>↓ json</button>
          <button style={{ fontFamily: CTok.sans, fontSize: 12.5, padding: '6px 12px', background: '#fff', color: CTok.ink1, border: `1px solid ${CTok.ink3}` }}>↓ md</button>
          <button style={{ fontFamily: CTok.sans, fontSize: 12.5, fontWeight: 600, padding: '6px 14px', background: CTok.ink0, color: '#fff', border: 'none' }}>Re-scan ↵</button>
        </div>
      </div>

      <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '320px 1fr', minHeight: 0 }}>
        {/* Grade panel */}
        <div style={{ padding: '28px 28px', borderRight: `1px solid ${CTok.ink5}`, display: 'flex', flexDirection: 'column' }}>
          <div style={{ fontFamily: CTok.mono, fontSize: 10, letterSpacing: '0.2em', color: CTok.ink3 }}>GRADE</div>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginTop: 4 }}>
            <div style={{ fontFamily: CTok.sans, fontSize: 184, fontWeight: 800, color: CTok.ink0, letterSpacing: '-0.07em', lineHeight: 0.78 }}>B+</div>
          </div>
        ...
        <div style={{ padding: '20px 26px', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ flex: 1, marginTop: 14, padding: '18px 22px', background: '#fff', border: `1px solid ${CTok.ink5}`, overflow: 'auto' }}>
            <Lattice cellSize={16} />
          </div>
        </div>
      </div>
    ...
    <div style={{ width: '100%', height: '100%', background: CTok.paper, display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '1fr 1fr', minHeight: 0 }}>
        ...
        <div style={{ padding: '22px 28px', borderRight: `1px solid ${CTok.ink5}`, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ marginTop: 12, padding: '20px 24px', background: '#fff', border: `1px solid ${CTok.ink5}` }}>
        ...
        <div style={{ padding: '24px 32px', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ marginTop: 4, padding: '12px 14px', background: '#fff', borderLeft: `3px solid ${CTok.ink0}`, fontFamily: CTok.sans, fontSize: 13.5, color: CTok.ink1, lineHeight: 1.55 }}>
          ...
          <div style={{ marginTop: 14, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <div>
              ...
              <pre style={{ margin: '4px 0 0', padding: '10px 12px', background: '#fff', border: `1px solid ${CTok.ink5}`, fontFamily: CTok.mono, fontSize: 10.5, lineHeight: 1.5, color: CTok.ink1, overflow: 'hidden' }}>{`{ "method": "tools/call",
  "params": {
    "name": "repo.read_file" } }
// scope granted = ["read"]
// scope required = ["write"]`}</pre>
            ...
            <div>
              ...
              <pre style={{ margin: '4px 0 0', padding: '10px 12px', background: '#fff', border: `1px solid ${CTok.ink0}`, fontFamily: CTok.mono, fontSize: 10.5, lineHeight: 1.5, color: CTok.ink1, overflow: 'hidden' }}>{`HTTP/1.1 200 OK
{ "result": {
    "content": [{ "type": "text",
      "text": "wrote 3 lines" }] } }
// expected -32002`}</pre>
          ...
          <pre style={{ marginTop: 4, padding: '12px 14px', background: CTok.ink0, color: '#e9e8df', fontFamily: CTok.mono, fontSize: 10.5, lineHeight: 1.55, flex: 1, overflow: 'hidden' }}>{`const required = tool.scopes ?? [];
  ...
  return rpc.error(-32002, 'insufficient_scope', {
    required, granted
          ...
          <div style={{ marginTop: 10, display: 'flex', gap: 8 }}>
            <button style={{ fontFamily: CTok.sans, fontSize: 12.5, fontWeight: 600, padding: '8px 14px', background: CTok.ink0, color: '#fff', border: 'none' }}>Open §7.4 in Learn →</button>
            <button style={{ fontFamily: CTok.sans, fontSize: 12.5, padding: '8px 14px', background: '#fff', color: CTok.ink1, border: `1px solid ${CTok.ink3}` }}>Copy fix</button>
          </div>
        </div>
      </div>
    </div>
...
function C_BuildCanvas() {
  ...
  const slot = (label, sub, filled, accent, k) => (
    <div key={k ?? label} style={{
      padding: 12, background: filled ? '#fff' : CTok.ink7,
  ...
  return (
    <div style={{ width: '100%', height: '100%', background: CTok.paper, display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ padding: '14px 40px 10px', display: 'flex', alignItems: 'baseline', gap: 18, borderBottom: `1px solid ${CTok.ink5}` }}>
        ...
        <span style={{ flex: 1 }} />
        <button style={{ fontFamily: CTok.sans, fontSize: 12.5, fontWeight: 600, padding: '6px 14px', background: CTok.ink0, color: '#fff', border: 'none' }}>Generate →</button>
      </div>
      ...
      <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 340px', minHeight: 0, gap: 1, background: CTok.ink5 }}>
        ...
        <div style={{ background: CTok.paper, padding: '18px 18px', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
          ...
          <textarea defaultValue="Read a single file from a GitHub repository at the given ref." style={{ marginTop: 4, padding: '7px 9px', fontFamily: CTok.sans, fontSize: 12, border: `1px solid ${CTok.ink4}`, background: '#fff', resize: 'none', minHeight: 46, boxSizing: 'border-box' }} />
    ...
    <div style={{ width: '100%', height: '100%', background: CTok.paper, display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, padding: '24px 40px', display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: 32, minHeight: 0 }}>
        <div style={{ overflow: 'hidden' }}>
          ...
          <div style={{ marginTop: 14, fontFamily: CTok.mono, fontSize: 11, background: '#fff', border: `1px solid ${CTok.ink5}` }}>
            ...
            {files.map((f, i) => (
              <div key={f[0]} style={{ display: 'grid', gridTemplateColumns: '24px 1fr 60px', padding: '6px 12px', borderBottom: i < files.length - 1 ? `1px solid ${CTok.ink6}` : 'none', alignItems: 'center', background: i % 2 ? CTok.ink7 : '#fff' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column' }}>
          ...
          <button style={{ marginTop: 18, fontFamily: CTok.sans, fontSize: 15, fontWeight: 700, padding: '14px 18px', background: CTok.ink0, color: '#fff', border: 'none', textAlign: 'left' }}>↓ &nbsp; Download .zip &nbsp; · &nbsp; 11.7 kB</button>
          <button style={{ marginTop: 8, fontFamily: CTok.sans, fontSize: 12.5, padding: '10px 14px', background: '#fff', color: CTok.ink1, border: `1px solid ${CTok.ink3}`, textAlign: 'left' }}>Copy curl one-liner</button>
          <button style={{ marginTop: 8, fontFamily: CTok.sans, fontSize: 12.5, padding: '10px 14px', background: '#fff', color: CTok.ink1, border: `1px solid ${CTok.ink3}`, textAlign: 'left' }}>Dockerfile</button>

          <div style={{ marginTop: 18, padding: 14, background: '#fff', border: `1px solid ${CTok.ink5}`, fontFamily: CTok.mono, fontSize: 11, color: CTok.ink1, lineHeight: 1.7 }}>
  ...
  return (
    <div style={{ width: '100%', height: '100%', background: CTok.paper, display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ padding: '14px 40px 10px', display: 'flex', alignItems: 'center', gap: 14, borderBottom: `1px solid ${CTok.ink5}` }}>
        <input placeholder="search · "  defaultValue="scope" style={{ flex: 1, padding: '10px 14px', fontFamily: CTok.mono, fontSize: 13, border: `1px solid ${CTok.ink3}`, background: '#fff', boxSizing: 'border-box' }} />
      ...
      <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '260px 1fr 280px', minHeight: 0 }}>
        ...
        <div style={{ padding: '24px 32px', overflow: 'hidden' }}>
          ...
          <pre style={{ marginTop: 6, padding: '14px 16px', background: '#fff', border: `1px solid ${CTok.ink5}`, fontFamily: CTok.mono, fontSize: 11.5, lineHeight: 1.55, color: CTok.ink1, overflow: 'hidden' }}>{`{
  "error": {
    "code": -32002,
    ...
    "data": { "required": ["write"], "granted": ["read"] }
        ...
        <div style={{ padding: '18px 18px', borderLeft: `1px solid ${CTok.ink5}`, overflow: 'hidden' }}>
          ...
          <div style={{ marginTop: 22, padding: 14, background: '#fff', border: `1px solid ${CTok.ink3}` }}>
            ...
            <button style={{ marginTop: 8, fontFamily: CTok.sans, fontSize: 12, fontWeight: 600, padding: '6px 10px', background: CTok.ink0, color: '#fff', border: 'none' }}>← back to report</button>
          </div>
        </div>
      </div>
    </div>
      ...
      <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '1fr 1fr', minHeight: 0, gap: 1, background: CTok.ink5 }}>
        ...
        <div style={{ background: CTok.paper, padding: '32px 36px', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ marginTop: 32 }}>
            {/* Empty lattice */}
            <div style={{ padding: '18px 20px', background: '#fff', border: `1px solid ${CTok.ink5}` }}>
          ...
          <div style={{ marginTop: 14, display: 'flex', gap: 8 }}>
            <input placeholder="http://localhost:3845/mcp" style={{ flex: 1, padding: '11px 14px', fontFamily: CTok.mono, fontSize: 13, border: `1px solid ${CTok.ink3}`, background: '#fff', boxSizing: 'border-box' }} />
            <button style={{ fontFamily: CTok.sans, fontSize: 13.5, fontWeight: 600, padding: '11px 18px', background: CTok.ink0, color: '#fff', border: 'none' }}>Scan ↵</button>
          </div>
        </div>
        ...
        <div style={{ background: CTok.paper, padding: '32px 36px', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ marginTop: 22, padding: '18px 20px', background: '#fff', border: `1px solid ${CTok.ink5}`, position: 'relative' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          ...
          <div style={{ marginTop: 14, display: 'flex', gap: 8 }}>
            <button style={{ fontFamily: CTok.sans, fontSize: 13.5, fontWeight: 600, padding: '10px 18px', background: CTok.ink0, color: '#fff', border: 'none' }}>Retry ↵</button>
            <button style={{ fontFamily: CTok.sans, fontSize: 13.5, padding: '10px 18px', background: '#fff', color: CTok.ink1, border: `1px solid ${CTok.ink3}` }}>Edit endpoint</button>
          </div>
        </div>
      </div>
    </div>
